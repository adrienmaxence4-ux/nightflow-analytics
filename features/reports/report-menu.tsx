      ...
      {open &&
        ...
        createPortal(
          <div
            ...
            className="z-50 w-60 overflow-hidden rounded-xl border border-line bg-panel p-1.5 shadow-[0_16px_40px_-12px_rgba(0,0,0,0.7)] "
          >
            {FORMATS.map((f) => (
              <button
                key={f.id}
              ...
              >
                <f.icon className="h-4 w-4 flex-none text-accent-text" />
                <span>
                  <span className="block text-[13px] font-bold">{f.label}</span>
                  <span className="block text-[11px] text-ink3">{f.hint}</span>
                </span>
              </button>
            ))}
          </div>,
